...
static int get_replica_exchange(FILE *fplog,const gmx_multisim_t *ms,
                                ...
                                int step,real time,int num_grps)
{
    int  m,i,a,b;
   ...
   double probsum = 0.0;
    ...
    int num_grp = num_grps;
    double num_grp_d = (double)num_grps;
    int k;
    double low_prob,high_prob;
    low_prob = 0.0;
    high_prob = 0.0;
    ...
    for(i=1; i<re->nrepl;i++){
     ...
     k = (int)roundf(rando(&(re->seed))*num_grp_d);

          group[i] = k;
          sum_of_prob[group[i]] = sum_of_prob[group[i]] + prob[i];
    ...
    {    
      ...
      if( sum_of_prob[i-1] < xi*probsum && xi*probsum <= sum_of_prob[i]) {
